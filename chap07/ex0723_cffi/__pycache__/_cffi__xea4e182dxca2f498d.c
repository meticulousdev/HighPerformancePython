...
static int _cffi_var_Pnt(PyObject *lib)
{
  PyObject *o;
  int res;
  struct Point * i;
  ...
  if (o == NULL)
    return -1;
  res = PyObject_SetAttrString(lib, "Pnt", o);
  Py_DECREF(o);
  if (res < 0)
    return -1;
  return ((void)lib,0);
...
PyMODINIT_FUNC
PyInit__cffi__xea4e182dxca2f498d(void)
{
  PyObject *lib;
...
PyMODINIT_FUNC
init_cffi__xea4e182dxca2f498d(void)
{
  PyObject *lib;
  lib = Py_InitModule("_cffi__xea4e182dxca2f498d", _cffi_methods);
  if (lib == NULL)
    return;
  if (((void)lib,0) < 0 || _cffi_init() < 0)
    return;
