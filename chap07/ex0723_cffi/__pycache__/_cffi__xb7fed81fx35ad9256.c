...
static int _cffi_var_do_calculation(PyObject *lib)
{
  PyObject *o;
  int res;
  struct Point * i;
  ...
  if (o == NULL)
    return -1;
  res = PyObject_SetAttrString(lib, "do_calculation", o);
  Py_DECREF(o);
  if (res < 0)
    return -1;
  return ((void)lib,0);
...
{
  return _cffi_var_do_calculation(lib);
}
...
PyMODINIT_FUNC
PyInit__cffi__xb7fed81fx35ad9256(void)
{
  PyObject *lib;
...
PyMODINIT_FUNC
init_cffi__xb7fed81fx35ad9256(void)
{
  PyObject *lib;
  lib = Py_InitModule("_cffi__xb7fed81fx35ad9256", _cffi_methods);
  if (lib == NULL)
    return;
  if (((void)lib,0) < 0 || _cffi_init() < 0)
    return;
